...
func NewMap(m map[string]any, val *validator.Validate, options ...ResolverOption) Resolver {
	r := &mapResolver{m: m, validator: val}
	r.applyOptions(append([]ResolverOption{WithKey("map")}, options...)...)
...
func (r mapResolver) Resolve(path []string, valueType reflect.Type) (any, bool, error) {
	...
	for i, p := range path {
		...
		if i >= len(path)-1 {
			...
			} else if sliceV, sliceOk := rawV.([]any); sliceOk {
		...
		mapV, mapOk := rawV.(map[string]any)
		if !mapOk {
			if rawV == nil {
				return nil, false, nil
			}
