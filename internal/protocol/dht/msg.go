...
//
// A KRPC message is a single dictionary with two keys common to every message and additional keys
...
// may be correlated with multiple queries to the same node. The transaction ID should be encoded as a
// short string of binary numbers, typically 2 characters are enough as they cover 2^16 outstanding queries.
...
type Msg struct {
	// Q Query method (one of 4: "ping", "find_node", "get_peers", "announce_peer")
	Q string `bencode:"q,omitempty"`
	// A named arguments sent with a query
	A *MsgArgs `bencode:"a,omitempty"`
	// T required: transaction ID
	T string `bencode:"t"`
	// Y required: type of the message: q for QUERY, r for RESPONSE, e for ERROR
	Y string `bencode:"y"`
	// R RESPONSE type only
	R *Return `bencode:"r,omitempty"`
	// E ERROR type only
	E  *Error   `bencode:"e,omitempty"`
	IP NodeAddr `bencode:"ip,omitempty"`
	// ReadOnly BEP 43. Sender does not respond to queries.
	ReadOnly bool `bencode:"ro,omitempty"`
	// ClientID https://www.libtorrent.org/dht_extensions.html
